...
def test_sparse_episode_ids_map_to_selected_arrow_positions():
    dataset = LatentLeRobotDataset.__new__(LatentLeRobotDataset)
    dataset.episode_position = {108: 0, 355: 1}
    dataset.episode_data_index = {
        'from': torch.tensor([0, 467]),
        'to': torch.tensor([467, 900]),
    }

    assert dataset._get_global_idx(108, 12) == 12
    assert dataset._get_global_idx(355, 12) == 479


def test_dense_text_encoder_prefers_local_instruction_embedding():
    dataset = LatentLeRobotDataset.__new__(LatentLeRobotDataset)
    dataset.used_video_keys = ['camera']
    dataset.config = SimpleNamespace(env_type='franka', text_encoder_type='umt_dense')
    dataset.cfg_prob = 0.0
    dataset.empty_emb = torch.full((1,), -1.0)
    latent = torch.zeros(1, 4)
    data = {
        'camera.latent': latent,
        'camera.latent_num_frames': 1,
        'camera.latent_height': 1,
        'camera.latent_width': 1,
        'camera.local_instruction_emb': torch.full((2, 3), 7.0),
        'camera.text_emb': torch.full((2, 3), 2.0),
    }

    output = dataset._cat_video_latents(data)

    torch.testing.assert_close(output['text_emb'], data['camera.local_instruction_emb'])
