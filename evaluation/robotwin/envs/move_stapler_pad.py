...
class move_stapler_pad(Base_Task):
    ...
    def check_success(self):
        ...
        # Success means the stapler is released on the colored pad. Use physical
        # contact with the pad instead of enforcing a specific stapler yaw.
        pad_half_size = np.array([0.055, 0.03])
        xy_margin = np.array([0.025, 0.025])
        xy_on_pad = np.all(np.abs(stapler_pos[:2] - pad_pos[:2]) < (pad_half_size + xy_margin))
        stapler_pad_contact = self.check_actors_contact(self.stapler.get_name(), self.pad.get_name())

        return (xy_on_pad and stapler_pad_contact and self.robot.is_left_gripper_open()
